...
import pg from 'pg';
const { Pool } = pg;
const DATABASE_URL = process.env.DATABASE_URL || 'postgres://loomo:loomo_dev_password@localhost:5432/loomo_db';
const pool = new Pool({
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === 'save_song') {
            const { id, title, artist, source, duration, bpm, file_path, metadata, tracks, tags } = args;
            await client.query('BEGIN');
            // 1. Upsert song
            await client.query(`INSERT INTO songs (id, title, artist, source, duration, bpm, file_path, metadata, updated_at)
         ...
         ON CONFLICT (id) DO UPDATE SET
           ...
           updated_at = NOW()`, [id, title, artist || null, source || 'local', duration || 0, bpm || 120, file_path || null, JSON.stringify(metadata || {})]);
            // 2. Upsert tracks if provided
            if (Array.isArray(tracks)) {
                await client.query('DELETE FROM song_tracks WHERE song_id = $1', [id]);
                for (const t of tracks) {
                    await client.query(`INSERT INTO song_tracks (song_id, track_index, name, instrument, program, hand)
             VALUES ($1, $2, $3, $4, $5, $6)`, [id, t.track_index, t.name, t.instrument || 'acoustic_grand_piano', t.program || 0, t.hand || 'none']);
                }
            }
            ...
            if (Array.isArray(tags)) {
                ...
                for (const tag of tags.slice(0, 5)) {
                    ...
                    if (cleanTag) {
                        await client.query(`INSERT INTO song_tags (song_id, tag) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [id, cleanTag]);
                    }
                }
            }
        ...
        if (name === 'record_session') {
            const { session_id, song_id, user_id, timestamp, total_score, accuracy, streak_max, hit_metrics, duration_metrics } = args;
            await client.query('BEGIN');
            // 1. Insert session
            await client.query(`INSERT INTO practice_sessions (id, song_id, user_id, timestamp, total_score, accuracy, streak_max)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         ON CONFLICT (id) DO NOTHING`, [session_id, song_id, user_id || null, timestamp, total_score || 0, accuracy, streak_max || 0]);
            // 2. Insert hit metrics
            await client.query(`INSERT INTO session_hit_metrics (session_id, perfect, early, late, missed, error)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (session_id) DO NOTHING`, [session_id, hit_metrics.perfect || 0, hit_metrics.early || 0, hit_metrics.late || 0, hit_metrics.missed || 0, hit_metrics.error || 0]);
            // 3. Insert duration metrics
            await client.query(`INSERT INTO session_duration_metrics (session_id, duration_held, average_duration_score)
         VALUES ($1, $2, $3)
         ON CONFLICT (session_id) DO NOTHING`, [session_id, duration_metrics.duration_held || 0, duration_metrics.average_duration_score || 0]);
            await client.query('COMMIT');
            return {
